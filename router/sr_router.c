...
int verify_ip_packet(sr_ip_hdr_t* header) {
    ...
    header->ip_sum = 0;
    uint16_t new_chekcsum = cksum(head, header->ip_h1 * 4);
...
void send_icmp_msg(struct sr_instance* sr, uint8_t* packet, unsigned int len, uint8_t type, uint8_t code) {
    ...
    switch(type) {
        case icmp_type_echo_reply: {
            ...
            /* create ICMP header */
            sr_icmp_hdr_t* icmp_hdr = (sr_icmp_hdr_t*)(packet + sizeof(sr_ethernet_hdr_t) + (ip_hdr->ip_h1 * 4));
            icmp_hdr->icmp_type = type;
            icmp_hdr->icmp_code = code;

            /* recompute ICMP checksum */
            icmp_hdr->icmp_sum = 0;
            icmp_hdr->icmp_sum = cksum(icmp_hdr, ntohs(ip_hdr->ip_len) - (ip_hdr->ip_h1 * 4));
        ...
        case icmp_type_dest_unreachable: {
            ...
            sr_icmp_t3_hdr_t* icmp_hdr = (sr_icmp_t3_hdr_t*)(new_packet + sizeof(sr_ethernet_hdr_t) + (ip_hdr->ip_h1 * 4));
            ...
            /* set ip_hdr */
            new_ip_hdr->ip_v    = 4;
            new_ip_hdr->ip_h1   = sizeof(sr_ip_hdr_t) / 4;
            new_ip_hdr->ip_tos  = 0;
